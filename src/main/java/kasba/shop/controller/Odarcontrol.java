...
public ResponseEntity<Odar> addodar(@RequestBody List<Odar> lst){
 ...
 Date od=sdf.parse(o.getStringorderdate());   
	...
	if(o.getCompany().length()<1){
		
		wrong =wrong+", company name is empty";
	}
	
	
	if(!urr.existsById(o.getShopid())){
		wrong =wrong+", wrong shop id ";
	}
...
if(prr.existsByCompanyAndNameAndCode(p.getCompany(),p.getName(),p.getCode())) {
		...
		p.setCompany(o.getCompany());
		p.setTrid(o.getTrid());
		p.setDue(0);p.setShopid(o.getShopid());	
...
or.setTrid(o.getTrid());or.setCompany(o.getCompany()); or.setOrderdate(o.getOrderdate());or.setShopid(o.getShopid());
...
List<Stock> st = str.findByShopidAndCompanyAndNameAndCode(or.getShopid(),or.getCompany(),or.getName(),or.getCode()); 
 if(st.isEmpty()){
		   ...
		   mp.setAvg(or.getUnitprice());mp.setMax(or.getUnitprice());mp.setMin(or.getUnitprice());
			...
			mpr.save(mp); // and save uniquely no duplicate product will be stored for same shopid
